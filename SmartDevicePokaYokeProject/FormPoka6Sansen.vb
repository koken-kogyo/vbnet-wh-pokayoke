...
Public Class FormPoka6Sansen

    ' このWindowのインスタンス
    Public Shared FormPoka6Instance As FormPoka6Sansen
    ...
    Private Sub btnF2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnF2.Click
        If getRecordCount(tblNamePoka6) = 0 Then
            MessageBox.Show("送信するデータが存在しません", "警告")
            Exit Sub
        End If

        ' CSVファイル作成
        If SQLite2CSV(tblNamePoka6) <> 0 Then
            MessageBox.Show("CSVファイルの作成に失敗しました")
            Exit Sub
        End If

        ' ファイル転送
        Dim dialog As New FormTransmitting(tblNamePoka6)
        If (System.Windows.Forms.DialogResult.OK = dialog.ShowDialog()) Then

            'DB Delete
            If deletePokaX(tblNamePoka6) = 0 Then
                txtClear()
            Else
                MessageBox.Show("CSVファイルの作成に失敗しました")
                Exit Sub
    ...
    Private Sub btnF3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnF3.Click
        Dim frm As Form = New FormPokaHistory(tblNamePoka6, txtTANCD.Text)
        frm.ShowDialog()
        Dim cnt As Int32 = getRecordCount(tblNamePoka6)
        If cnt = 0 Then
            txtClear()
        Else
            lblCount.Text = cnt
    ...
    Private Sub txtClear()
        txtHMCD.Text = ""
        txtTKHMCD.Text = ""
        lblCount.Text = getRecordCount(tblNamePoka6)
        txtHMCD.Focus()
    End Sub
    ...
    Private Sub Judge()
        ...
        ElseIf Strings.Right(txtHMCD.Text, 2) = "-S" And _
            Strings.Left(txtHMCD.Text, txtHMCD.Text.Length - 2).Replace("-", "") = _TKHMCD.Replace("-", "") Then
            isOK = True

        ElseIf Strings.Right(txtHMCD.Text, 2) = "-1" And _
            Strings.Left(txtHMCD.Text, txtHMCD.Text.Length - 2).Replace("-", "") = _TKHMCD.Replace("-", "") Then
            isOK = True
        ...
        If isOK Then
            ...
            rec.DATABASE = "-"
            rec.ODRNO = ""
            ret = insertPokaX(tblNamePoka6, rec)
